...
# 6095 /dev/inf 02.2023 (Уровень: Базовый) 🌶️
res = set()  # когда i==k получаютя дубли
alf = [[''], [*map(str, range(10))], [*map(str, range(10, 100))]]
for a in range(3):
    for b in range(3-a):
        num = [f'{i}15{k}7424' for i in alf[a] for k in alf[b]]
        for i in num:
            r = [not int(i)%k for k in (111,113,127)]
            if sum(r) == 1:
                res |= {(int(i), (111,113,127)[r.index(1)])}
[print(i[0], i[0]//i[1]) for i in sorted(res)]
"""
1587424 14048
15147424 134048
15227424 137184
...
# раздожить число на простые множители ✅
def mul(n):
	d = []
	for i in range(2, int(n**0.5 + 1)):
		while not n % i:
...
c = 5
for n in range(8_996_452, 10**10):
    div = mul(n)
    if len(div) == 2 and all(str(i).count('3') == 2 for i in div):
        print(n, div[1])
        c -= 1
    if not c:
        break
