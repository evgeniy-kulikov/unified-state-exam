...
def f(st, en, w='+'):
    ...
    if st == en:
        return 1
    return ((f(st*2, en, w+'*') if w[-1] != '*' else 0) +
            (f(st*3, en, w+'*') if w[-1] != '*' else 0) +
            f(st+1, en, w+'+'))
print(f(1, 30))  # 211

# через численный параметр
def f(st, en, p=0):
    if st > en:
        return 0
    if st == en:
        return 1
    return ((f(st*2, en, p+1) if not p else 0) +
            (f(st*3, en, p+1) if not p else 0) +
            f(st+1, en, 0))
print(f(1, 30))  # 211

# короче
def f(st, en, p=0):  # p=1 если предыдущая команда была умножением, иначе 0
    if st > en:
        return 0
    if st == en:
        return 1
