...
простые числа — это натуральные числа больше 1, которые имеют ровно два различных делителя: единицу и само число. 
Число 1 имеет только один делитель — себя, что не соответствует этому условию
...
# Разложить число на простые множители (множители ПРОСТЫЕ числа. Любое число можно разложить) ✅
def mul(n):
    d = []
    for i in range(2, int(n**0.5 + 1)):
        while not n % i:
